...
describe('Login Page', () => {
  ...
  it('Inicia sesión, debe almacenar el token en localStorage y redirige la pagina', fakeAsync(() => {
    const inputEmail = SELECTORS.LOGIN.inputEmail();
    const inputPassword = SELECTORS.LOGIN.inputPassword();
  ...
  it('El token ya esta almacenado y debe redirigir al la pagina principal', () => {
    const { token } = new LoginMockService().login(null);
